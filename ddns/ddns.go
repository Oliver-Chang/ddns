...
func (d *DDNS) Deamon() {
	...
	schedTime := 10 * time.Second
	go d.fetchLatestIPv6(ipChan, schedTime)
	for {
		select {
		case ipv6, ok := <-ipChan:
			if ok {
				logger.Logger.Info("Update DNS Record", zap.String("ipv6", ipv6))
...
func (d *DDNS) updateRecord(ipv6 string) error {
	...
	if recordLen := len(records); recordLen > 1 {
		...
		logger.Logger.Info("Will update subdomain record")
...
getip:
	...
	if !iputil.IsIPv6(ip) {
		t := time.Duration(1<<uint32(rand.Intn(6))) * time.Second
	...
	if ip != oldIP {
		...
		logger.Logger.Error("IP address is not change", zap.String("ip", ip), zap.String("old_ip", oldIP))
	}
